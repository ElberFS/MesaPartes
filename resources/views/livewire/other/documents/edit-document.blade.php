<div class="min-h-screen bg-gray-100 p-4 flex items-center justify-center">
    <div class="w-full max-w-2xl bg-white shadow-lg rounded-lg p-6 space-y-6">
        <h2 class="text-2xl font-bold text-gray-900 text-center">Editar Documento: {{ $document->code }}</h2>
        ...
        <form wire:submit.prevent="update" class="space-y-4">
            {{-- Código de Documento --}}
            <div>
                <label for="code" class="block text-sm font-medium text-gray-700">Código de Documento</label>
            ...
            @if ($origin_type === 'external')
                <div class="space-y-4 border border-gray-200 p-4 rounded-md bg-gray-50">
                    <h3 class="text-lg font-semibold text-gray-800">Detalles de Origen Externo</h3>
                    ...
                    <div>
                        <label for="external_contact_person" class="block text-sm font-medium text-gray-700">Persona de Contacto Externa</label>
                    ...
                    <div>
                        <label for="external_contact_role" class="block text-sm font-medium text-gray-700">Cargo de Contacto Externo</label>
            ...
            {{-- Encargado de Origen --}}
            <div>
                <label for="origin_in_charge" class="block text-sm font-medium text-gray-700">Encargado de Origen</label>
            ...
            {{-- Archivo Adjunto Actual y Carga de Nuevo Archivo --}}
            <div>
                <label class="block text-sm font-medium text-gray-700">Archivo Adjunto</label>
                @if ($currentFilePath)
                    <p class="mt-1 text-sm text-gray-600">Archivo actual: <a href="{{ $currentFilePath }}" target="_blank" class="text-indigo-600 hover:text-indigo-900">Ver archivo</a></p>
                @else
                    <p class="mt-1 text-sm text-gray-600">Ningún archivo adjunto actualmente.</p>
                @endif
                <label for="newFile" class="block text-sm font-medium text-gray-700 mt-2">Subir Nuevo Archivo (PDF, DOC, DOCX - Max 10MB)</label>
                <input type="file" id="newFile" wire:model="newFile"
                       class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none">
                @error('newFile') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                <div wire:loading wire:target="newFile" class="mt-2 text-sm text-indigo-600">Subiendo archivo...</div>
            </div>
